...
if __name__ == "__main__":
    ...
    getToken()
    stream = getStreamKeyFromSongIDs(ID) #Get the StreamKey for the selected song
    for k,v in stream.iteritems():
        stream=v
    if stream == []:
        print "Failed"
        exit()
    cmd = 'wget --post-data=streamKey=%s -O "%s/%s.mp3" "http://%s/stream.php"' % (stream["streamKey"], folder,NAME, stream["ip"]) #Run wget to download the song
    print cmd
    ...
    markSongDownloadedEx(stream["ip"], ID, stream["streamKey"]) #This is the important part, hopefully this will stop grooveshark from banning us.
    exit()